...
| R-17 | **Surface d'exposition du VPS** (SSH ouvert, ports de service, secrets sur disque, conteneur en root). | 2 | 3 | 6 | Pare-feu par défaut fermé ; SSH par clé uniquement, sans mot de passe ni root ; conteneur non-root ; secrets en fichier `600` hors du dépôt ; en-têtes HTTP de sécurité ; audit en Phase 14. | 14 |
...
| H-01a | Le VPS dispose d'au moins **2 vCPU / 2 Go de RAM** et de Docker. | En dessous, le build ne doit pas être fait sur le VPS (il ne l'est pas : l'image est construite en CI) et il faut surveiller la RAM du conteneur. Impact faible. |
...
| H-03 | Le CV est un **PDF statique versionné** dans le dépôt (pas de génération dynamique). | Génération à la volée = travail supplémentaire, repoussé hors périmètre v1. |
